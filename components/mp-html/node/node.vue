<template>
  <view @tap="nodeTap" :id="attrs.id" :class="'_block _'+name+' '+attrs.class" :style="(ctrl.root?'border:0px solid black;padding:0px;display:block;':'')+attrs.style">
    <block v-for="(n, i) in childs" v-bind:key="i">
      ...
      <text v-else-if="n.type==='text'&&!ctrl['e'+i]" :data-i="i" :user-select="n.us" :decode="!opts[4]" @tap="editStart">{{n.text}}
        <text v-if="!n.text" style="color:gray">{{opts[5]||placeholderText}}</text>
      </text>
      <text v-else-if="n.type==='text'&&ctrl['e'+i]===1" :data-i="i" style="border:0px dashed black;min-width:50px;width:auto;padding:0px;display:block" @tap.stop="editStart">{{n.text}}
        <text v-if="!n.text" style="color:gray">{{opts[5]||placeholderText}}</text>
      </text>
      <textarea v-else-if="n.type==='text'" style="border:0px dashed black;min-width:50px;width:auto;padding:0px; font-size: 36rpx" auto-height maxlength="-1" :focus="ctrl['e'+i]===3" :value="n.text" :data-i="i" @input="editInput" @blur="editEnd" />
      ...
      <!-- #endif -->
      <!-- #ifndef APP-PLUS -->
      <video :show-center-play-btn="!opts[4]" @tap="mediaTap" v-else-if="n.name==='video'" :id="n.attrs.id" :class="n.attrs.class" :style="n.attrs.style" :autoplay="n.attrs.autoplay" :controls="n.attrs.controls" :loop="n.attrs.loop" :muted="n.attrs.muted" :object-fit="n.attrs['object-fit']" :poster="n.attrs.poster" :src="n.src[ctrl[i]||0]" :data-i="i" @play="play" @error="mediaError" />
      <!-- #endif -->
      <!-- #ifdef H5 || APP-PLUS -->
...
export default {
  ...
  methods:{editStart (e) {  
      if (this.opts[4]) {
		...
		this.root.$emit('editInput', e.detail.value)
